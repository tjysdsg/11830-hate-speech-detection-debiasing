...
def load_and_cache_examples(data, args, tokenizer):
    if args.line_by_line:
        ...
        #print(train_dataset['attribute0'])
...
def split_data(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args):
    ...
    print("Size is "+str(args.dev_data_size))
...
def create_dataloader(args, datasets, tokenizer, train=False):
    ...
    for key, dataset in datasets.items():
        ...
        if train:
            #print("training size is "+str(args.train_batch_size))
...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    #print(train_datasets) LineByLineTextDataset object
    ...
    def get_hiddens_of_model(input):
        ...
        if args.model_type == 'roberta':
            ...
            outputs = model.albert(input)
        elif args.model_type == 'dbert':
            outputs = model.distilbert(input)
        elif args.model_type == 'electra':
            outputs = model.electra(input)
        elif args.model_type == 'gpt2':
            outputs = model.transformer(input)
    ...
    def forward(attributes_hiddens, dataloaders, key):
        ...
        if args.model_type == 'roberta':
            ...
            final_layer_hiddens = outputs.last_hidden_state
            if 'neutral' != key:
                with torch.no_grad():
                    original_outputs = original_model.distilbert(inputs)
                    final_layer_original_hiddens = original_outputs.last_hidden_state
                    all_layer_original_hiddens = original_outputs.hidden_states
                if args.token_loss:
                    token_predicts = model.classifier(final_layer_hiddens)
                    token_original = original_model.classifier(final_layer_original_hiddens)
        elif args.model_type == 'electra':
...
def main():
    ...
    if args.block_size <= 0:
        ...
        args.block_size = min(args.block_size, tokenizer.model_max_length) # max_len model_max_length
    ...
    #print("data file is "+str(args.data_file))
